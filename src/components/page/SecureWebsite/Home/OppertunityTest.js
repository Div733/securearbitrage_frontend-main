...
import { requestFlashLoan } from "../../../../api/contracts/FlashLoan/requestLoan";
import { transfer } from "../../../../api/contracts/ERC20/transfer";
import {
  CONTRACT_ADDRESS_SEPOLIA_AAVE_USDT,
  CONTRACT_ADDRESS_SEPOLIA_FLASH_LOAN,
} from "../../../../utils/address";
import { isConnected, switchNetwork } from "../Hooks/metamask";
import { useWallet } from "../Hooks/useConnectWallet";
import { humanToE6s } from "../../../../utils/conversions";
import WalletModal from "../Components/WalletModal";

const ethers = require("ethers");

const OppertuinityTest = () => {
  const {
    walletAddress,
    ethBalance,
    user,
  ...
  if (selectedOption == "Seek Loan") {
    ...
    loan = Array.from({ length: (1000 - 100 + 1) }, (_, i) => 100 + i);
  } else {
  ...
  const [loanAmount, setLoanAmount] = useState(
    loan[[Math.floor(Math.random() * loan?.length)]]?.toFixed(6)
  );
  ...
  const [exFeeB, setExFeeB] = useState((loanAmount / reducedPrice) * 0.001);
  ...
  const handleSubmit = async () => {
    ...
    if (inputValue.trim() === "") {
      ...
      setError("Please enter a number other than 0");
    } else {
      setError("");
      handleCloseModal();
      setInputValue("");
  ...
  const connection = async () => {
    let conn = await isConnected();
    console.log(": --------------------------");
    console.log(": connection -> conn", conn);
    console.log(": --------------------------");

    if (conn.status) {
      setUserAddress(conn.message);
    } else {
      // await connectWallet();
  ...
  let TEF = (parseFloat(exFee) + parseFloat(exFeeB) * increasedPrice).toFixed(
    6
  );
  ...
  useEffect(() => {
    let updateExFee;
    if (selectedServices[0] === "Pancake Swap")
      updateExFee = (loanAmount * 0.008).toFixed(6);
    else if (selectedServices[0] === "Biswap")
      updateExFee = (loanAmount * 0.008).toFixed(6);
    else if (selectedServices[0] === "Mdex")
      updateExFee = (loanAmount * 0.008).toFixed(6);
    else if (selectedServices[0] === "ApeSwap")
      updateExFee = (loanAmount * 0.008).toFixed(6);
  ...
  useEffect(() => {
    let updateExFeeB;
    if (selectedServices[1] === "Pancake Swap")
      updateExFeeB = (DAB * 0.008).toFixed(6);
    else if (selectedServices[1] === "Biswap")
      updateExFeeB = (DAB * 0.008).toFixed(6);
    else if (selectedServices[1] === "Mdex")
      updateExFeeB = (DAB * 0.008).toFixed(6);
    else if (selectedServices[1] === "ApeSwap")
      updateExFeeB = (DAB * 0.008).toFixed(6);
  ...
  const handleShow1 = async () => {
    if ((await isConnected()).status) {
      const fee = (inputValue * 0.05) / 100;
      console.log(": ------------------------------");
      console.log(": OppertuinityTest -> fee", fee);
      console.log(": ------------------------------");
      const feeNonHuman = humanToE6s(fee);
      console.log(": ----------------------------------------------");
      console.log(": OppertuinityTest -> feeNonHuman", feeNonHuman);
      console.log(": ----------------------------------------------");

      Swal.fire({
        text: `Initiating Fee Transfer of ${fee} USDT `,
        icon: "success",
        timer: "2000",
        showConfirmButton: false,
        background: "#2E292E",
      ...
      // tranferring 10 USDT to flash loan contract
      await transfer(
        CONTRACT_ADDRESS_SEPOLIA_AAVE_USDT,
        CONTRACT_ADDRESS_SEPOLIA_FLASH_LOAN,
        feeNonHuman
      ...
      Swal.fire({
        text: `Initiate the call to Contract for Loan from AAVE of ${inputValue} USDT `,
        icon: "success",
        timer: "2000",
        showConfirmButton: false,
        background: "#2E292E",
      ...
      await requestFlashLoan(humanToE6s(inputValue));
      // setShow(true);
      // setStepsVisible([]);
      // setCurrentStep(0);
      // startStepDisplay();
      ...
      <div className=" mt-0 pt-0">
        <div className=" mt-0 pt-0">
          ...
          >
            ...
            <div
              className="p-md-4 p-2 rounded mb-2"
              style={{ background: "rgb(62 64 96)" }}
            >
              <Table
              ...
              >
                <thead>
                  ...
                  >
                    <th style={{ width: "50%" }}>
                      <p className="d-flex flex-column flex-sm-row gap-1 p-0 m-0 responsive-text">
                ...
                <tbody className="text-white">
                  ...
                  >
                    <td>
                      <p className="d-flex flex-column flex-sm-row gap-1 mb-0 pb-0">
                        {" "}
                        <span>Low Price at</span>{" "}
                        <span style={{ color: "#4B0082" }} className="">
                          {selectedServices[0]}
                        </span>
                      </p>
                    </td>
                    <td>
                      <p className="d-flex flex-column flex-sm-row gap-1 p-0 m-0">
                        {" "}
                        <span>High Price at </span>{" "}
                        <span style={{ color: "#4B0082" }} className="">
                          {selectedServices[1]}
                        </span>
                      </p>
                  ...
                  <tr>
                    <td>
                      <p className="d-flex flex-column flex-sm-row gap-1 p-0 m-0">
                        {" "}
                        <span>{reducedPrice} USDT</span>
                      </p>
                    </td>
                    <td>
                      <p className="d-flex flex-column flex-sm-row gap-1 p-0 m-0">
                        {" "}
                        <span>{increasedPrice} USDT</span>
                      </p>
                    </td>
                  </tr>
                  <tr
                ...
                <div className="d-flex gap-5">
                  <label className="d-flex items-center gap-2">
                    <input
                      type="checkbox"
                      value="Own Funds"
                      checked={selectedOption === "Own Funds"}
                  ...
                  <label className="d-flex items-center gap-2">
                    <input
                      type="checkbox"
                      value="Seek Loan"
                      checked={selectedOption === "Seek Loan"}
                      onChange={handleCheckboxChange}
                      disabled={isStartPay}
                      className="h-5 w-5 text-blue-500"
                    />
            ...
            <Modal show={botModal} onHide={handleCloseBot} centered>
              ...
              >
                <p style={{ fontSize: "12px" }}>
                  The Auto Bot enables you to capitalize on all arbitrage
                  opportunities. Please configure the following settings.
                  Currently, you can only use your own funds for the Auto Bot. A
                  subscription fee is required, along with a $1 bot fee for each
                  execution, paid in BNB.
                </p>

                <p>Please complete the following:</p>

                <div className="d-flex align-items-center">
                  <span className="mr-2" style={{ width: "45px" }}>
                    Email:
                  </span>
                  <input
                    type="email"
                  ...
                  <button
                    className={`btn ${
                      isValidEmail ? "btn-primary" : "btn-secondary"
                    } btn-sm p-1 px-2 py-1 pl-0 ml-0`}
                    style={{
                      border: "none",
                ...
                <p style={{ fontSize: "12px" }}>
                  An email is required to provide updates after each execution
                  and to notify you when 24 hours have passed. Please restart
                  the auto bot every 24 hours.
                </p>
                  ...
                  <div className="d-flex gap-2 align-items-center">
                    ...
                    <input
                      ...
                      style={{
                        ...
                        height: "10px",
                        background:
                          "linear-gradient(to right, #000 0%, #FF6347 50%, #000 100%)",
                        borderRadius: "10px",
                        outline: "none",
                      }}
                    />
                ...
                <div className="d-flex align-items-center gap-2">
                  <span className="" style={{ width: "45px" }}>
                    Capital:
                  </span>
                  <input
                    type="number"
                ...
                <div className="d-flex align-items-start gap-2 mt-2">
                  <span className="d-flex flex-wrap" style={{ width: "150px" }}>
                    Pairs: <span style={{ fontSize: "12px" }}>(Max 3)</span>
                  </span>
                  <div className="d-flex gap-2 align-items-center mt-1 flex-md-row flex-column">
                    <div style={{ marginLeft: "0px" }}>
                      {" "}
                      {/* Add left margin for starting point */}
...
export default OppertuinityTest;

function InputAmont1() {
  return (
    <>
